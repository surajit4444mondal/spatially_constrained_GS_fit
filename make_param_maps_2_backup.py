...
def detect_discont(fitted, param_val,x,y,numx,numy,num_params,search_length,thresh):
	...
	if high_indx-low_indx+1<search_length or high_indy-low_indy+1<search_length:
		return 0
...
def find_new_params(points_to_remove,fitted,model,spectrum,sys_error,rms,param_val,numx,numy,num_params, low_indx,low_indy,high_indx,high_indy):
	...
	for m,point in enumerate(points_to_remove):
		...
		original_chi_square=new_params[m][num_params]
		inds=[]
		for i in range(-1,2):
			for j in range(-1,2):
				x1=x0+j
		...
		grad_chi_square=calc_grad_chisquare(low_indx,low_indy,high_indx,high_indy, numx,numy,num_params, param_val,fitted)			
		print (grad_chi_square)
		for n,param_ind in enumerate(inds):
			...
			grad_chi_square_temp=calc_grad_chisquare(low_indx,low_indy,high_indx,high_indy, numx,numy,num_params, param_val,fitted)		#calc_grad_chisquare(max(low_indx, x0-2),max(low_indy,y0-2),min(high_indx,x0+2),min(high_indy,y0+2), numx,numy,num_params, param_val,fitted)		
			if grad_chi_square_temp<grad_chi_square and chi_square<original_chi_square*2:
...
def remove_discont(spectral_cube, err_cube, fitted, param_val,numx,numy,num_params,search_length,thresh,max_dist_parameter_space, model):
	...
	for point_num,discont in enumerate(discont_list):
		...
		for discont1 in discont_list[point_num+1:]:
			if discont1[0]>=low_indx and discont[0]<=high_indx	and discont1[1]>=low_indy and discont[1]<=high_indy:
				discont1[0]=-1
				discont1[1]=-1
		break	
	return		
...
smooth_param_maps(spectrum.spectrum, spectrum.error, fitted, model.param_vals,spectrum_shape[3],spectrum_shape[2],5,smooth_length,discontinuity_thresh,max_dist_parameter_space, model.model)
'''
